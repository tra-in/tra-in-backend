...
def recommend(req: RecommendRequest):
    ...
    now = pd.to_datetime(req.now) if req.now else pd.Timestamp.now()
    ...
    df1 = pd.read_sql(q1, engine, params={
    ...
    if req.max_transfers >= 2:
        ...
        df2 = pd.read_sql(q2, engine, params={
            "from_name": req.from_name,
            "to_name": req.to_name,
            "now": now,
            "deadline_plus": deadline_plus,
...
def recommend_v2(req: RecommendRequest):
    raw = recommend(req)  # 기존 recommend 로직
